...
async function main() {
  ...
  const [owner, managerAccount, testAccount] = await getAccounts();
  ...
  // Change fee recipient
  let newFeeRecipient = testAccount.address;
  ...
  await setToken.setManager(testAccount.address);
  let oldManager = manager;
  manager = testAccount;

  // Update manager on StreamingFeeModule
  streamingFeeModule = streamingFeeModule.connect(manager.wallet);
