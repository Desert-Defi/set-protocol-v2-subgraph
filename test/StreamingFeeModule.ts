...
describe.only("StreamingFeeModule", () => {
  let owner: Account;
  let manager: Account;
  ...
  // addSnapshotBeforeRestoreAfterEach(); // this causes an `evm_revert` at test end
  ...
  describe("#accrueFee", async () => {
    ...
    it("updates manager", async () => {
      ...
      [newManager] = await getAccounts();
      // setToken = setToken.connect(manager.wallet);
      await setToken.setManager(newManager.address);
    });
